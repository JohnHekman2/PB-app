...
def run_batch_analysis(vector_store, llm, selected_areas, analysis_prompt, system_template):
    ...
    for i, area in enumerate(selected_areas):
        ...
        try:
            # 3. Invoke the chain
            response_dict = rag_chain.invoke(analysis_prompt)
            summary = response_dict['answer']
            
            # 4. Extract unique source file names from the context documents
            unique_sources = set()
...
if st.button("▶️ Start Batch Analyse (Samenvatting)", disabled=not selected_areas):
    ...
    with st.container():
        ...
        markdown_content += f"**Analyse Vraag:** {ANALYSIS_PROMPT.replace('\n', ' ')}\n"
        ...
        for area, result_dict in analysis_results.items():
            ...
            if result_dict['sources']:
                markdown_content += "**Gebruikte Bronnen (Documenten):**\n"
